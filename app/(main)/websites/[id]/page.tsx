    ...
    <div className="p-6 lg:p-10 space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <Card className="border-primary/10 shadow-sm">
                <CardHeader>
                  ...
                  <CardDescription>
                    Most visited paths and their relative popularity.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <TopPagesBarChart
                    data={sortedPages.map(([path, visits]) => ({ path, visits: visits as number }))}
                  />
                </CardContent>
              </Card>
                ...
                <CardContent className="p-0">
                  <div className="divide-y max-h-[400px] overflow-auto">
                    {website.visits.map((visit) => (
                      <div
                        key={visit.id}
                        className="p-4 flex flex-col gap-1 hover:bg-muted/50 transition-colors"
                      >
                        <div className="flex justify-between items-start">
                          ...
                          <span className="text-[10px] text-muted-foreground">
                            {format(new Date(visit.entryTime), "HH:mm:ss")}
                          </span>
                        </div>
                        <div className="flex items-center gap-3 text-xs">
                          <span className="flex items-center gap-1">
                            <MapPin className="h-3 w-3" /> {visit.city}
                            {visit.region && visit.region !== "Unknown"
                              ? `, ${visit.region}`
                              : ""}, {visit.country}
                          </span>
                          <span className="flex items-center gap-1">
                            <Monitor className="h-3 w-3" /> {visit.browser} on {visit.os}
                          </span>
                        </div>
                      </div>
                    ))}
              ...
              <Card className="border-primary/10 shadow-sm">
                ...
                <CardContent>
                  <StatsList
                    ...
                    data={sortedSources as [string, number][]}
                    total={totalVisits}
                  />
                </CardContent>
              </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <Card className="border-primary/10 shadow-sm">
                ...
                <CardContent>
                  <StatsList
                    title="Country"
                    data={sortedCountries as [string, number][]}
                    total={totalVisits}
                  />
                </CardContent>
              </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <Card className="border-primary/10 shadow-sm">
                ...
                <CardContent>
                  <StatsList
                    title="Device Type"
                    data={sortedDevices as [string, number][]}
                    total={totalVisits}
                  />
                </CardContent>
              </Card>
              ...
              <Card className="border-primary/10 shadow-sm">
                ...
                <CardContent>
                  <StatsList
                    title="Browser"
                    data={browserData.map((b) => [b.name, b.value] as [string, number])}
                    total={totalVisits}
                  />
                </CardContent>
              </Card>
      ...
      <Card className="border-primary/10 shadow-sm border-dashed">
        <CardHeader>
          ...
          <CardDescription>
            Paste this script into the <code>&lt;head&gt;</code> of your website to start tracking
            visits.
          </CardDescription>
        </CardHeader>
        <CardContent>
